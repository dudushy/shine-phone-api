...
const cleanEnvironment = `export const environment = {
  production: false,
  // Em dev, usa proxy local para evitar CORS. Em produção, usar URL direta da API.
  apiBaseUrl: '/api/',
  apiToken: '',
};
`;
 ...
 *
 * Uso: node scripts/load-env.mjs <comando> [argumentos...]
 */
async function loadEnvAndRun() {
  ...
  if (existsSync(envPath)) {
    ...
    console.log('[load-env] Variáveis de ambiente carregadas do .env');
  } else {
